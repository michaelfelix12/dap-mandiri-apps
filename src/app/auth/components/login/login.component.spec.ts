...
import { ApiResponse } from '../../../shared/models/api-response.mode';
  ...
  describe('6.2. Form Login ReactiveForm test', () => {
    describe('6.2.1 FormGroup and FormControl should be initialize', () => {
      it('should be successfully initialized', () => {
        ...
        expect(loginComponent.loginForm.get('email')).toBeInstanceOf(
          AbstractControl
        );

        expect(loginComponent.loginForm.get('password')).toBeDefined();
        expect(loginComponent.loginForm.get('password')).toBeInstanceOf(
          AbstractControl
        );
      });
    });
    ...
    describe('6.2.2. Password Form Control should be validated', () => {
      let passwordControl: AbstractControl;
      beforeEach(() => {
        passwordControl = loginComponent.loginForm.get(
          'password'
        ) as AbstractControl;
      });
